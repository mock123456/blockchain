...
def new_transaction():
    ...
    if not all(k in values for k in required):
        return "缺少参数", 400
    index = blockchain.BlockChain.new_transaction(values['sender'], values['recipient'],values['amount'])
...
def register_nodes():
    ...
    for node in nodes:
        blockchain.BlockChain.register_node(node)
    response = {
        'message': "新节点已被添加",
...
def consensus():
    replaced = blockchain.BlockChain.resolve_conflict()
    if replaced:
        response = {
